...
const BY_CODE = {
  ...
  '23001': 'На эту запись ссылается история занятий — удалять её нельзя.',
...
export function humanError(error, fallback = 'Не удалось выполнить действие') {
  ...
  // P0001 — код, который Postgres ставит нашим собственным сообщениям
  // из проверок в базе (raise exception без явного кода). Сам Postgres
  // им не пользуется, поэтому такой текст уже написан для человека
  // и его надо показать как есть, а не подменять общей фразой.
  if (code === 'P0001' && error.message) return String(error.message)
