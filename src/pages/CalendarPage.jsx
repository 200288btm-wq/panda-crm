...
function DayModal({ date, events: initialEvents, teachers = [], onClose, onNavigate, isAdmin, myTeacherName, onAttendanceChange, clients = [], studioId, clientStatuses = [], allClients = [], directions = [], onClientsChanged, trialRepeatPolicy = 'warn' }) {
  ...
  const toggle = async (clientId, dirId, ev) => {
    ...
    // Счётчик посещений здесь больше не трогаем.
    //
    // Раньше отсюда звали adjust_visited вторым запросом. Это и было местом
    // поломки: отметка ложилась, а поправка счётчика упиралась в права
    // на карточку клиента — и молча не проходила. Теперь счётчик ведёт
    // триггер в базе, в одной транзакции с самой отметкой: не бывает
    // состояния «отметка есть, счётчик не сдвинулся».
    markingRef.current.delete(key)
